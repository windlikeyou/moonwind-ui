...
const showPagination = computed(() => {
  return (
    ...
    (props.fetch || (props.dataSource && props.dataSource.length > 0))
  );
});
...
const processedColumns = computed(() => {
  ...
  if (props.rowSelection) {
    const selectionColumn: any = {
      ...
      suppressMenu: true,
      pinned:
        props.rowSelection.fixed === true
          ? "left"
          : props.rowSelection.fixed || null,
      cellRenderer:
...
const fetchData = async () => {
  ...
  try {
    const sortModel =
      gridApi.value?.getColumnState?.()?.filter((col) => col.sort) || [];
...
const onSelectionChanged = (event: SelectionChangedEvent<T>) => {
  ...
  const newSelectedRows = selectedNodes
    .map((node) => node.data!)
    .filter(Boolean);
